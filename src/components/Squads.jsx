...
const Squads = () => {
  const SquadRow = ({ title, members, accentColor, description }) => (
    <div className="bg-gray-950 border border-slate-800 rounded p-6 mb-8 shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-stretch gap-4 relative z-10">
        {members.map((member, index) => (
          <div key={member.name} className="flex-1 w-full bg-slate-900 border border-slate-700 rounded p-4 text-center relative group hover:border-slate-500 transition-colors">
